...
//     const userJWT = req.user;
//     const user = await Orders.findById(userJWT.id);
...
export const getAllOrders = async (req, res, next) => {
  try {
    ...
    if (user.role === 'admin') {
      ...
      orders = await Orders.find({}).populate('products.product');
    } else {
      // Fetch orders for the user
      orders = await Orders.find({ user: userJWT.id }).populate('products.product');
    }
